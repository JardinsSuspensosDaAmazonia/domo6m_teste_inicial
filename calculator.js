  ...
  $(".download").on("click", function () {
    ...
    $(this).attr(
      ...
      downloadUrl +
        "?figure=" +
        encodeURIComponent(
          fragmentRouter.fromParams(form.state, "prevent #hash update")
        )
    );
  });
})();
...
const productPalette = _.reduce(
  Palette.collections,
  function (palette, colors, type) {
    palette[type] = _.map(
      /*colors*/ Palette.collections.line,
      function (cssColor) {
        return {
...
const fragmentRouter = new Stringifier({
  format: [
    ...
    .concat(
      _.map(
        CONFIG.router && CONFIG.router.beforeRemovedList,
        (conf) => conf.route
      ) // doorGroup eg.etc.
    )
    .concat([
    ...
    .map((template) => {
      return _.reduce(
        ...
        (template, replace, search) =>
          template.split(search).join("(?:" + replace + ")"),
        template
      );
    }),
    ...
    .extend({
      ...
      alignTheBase: {
        ...
        string: function (align) {
          return align && this.partial != "1/1" && this.partial != "1/2"
            ? "Flat_" /*'Align_'*/
            : "";
        },
      },
      ...
      fullerenType: {
        ...
        string: function (type) {
          if (type)
            return (
              (type == "inscribed" ? "Inscribed_" : "Described_") +
              "Fulleren_on_"
            );
          return "";
        },
      ...
      connType: {
        value: function (string) {
          var matches =
            /^(Piped|Joint|Cone|Semicone|Nose|GoodKarma|)(.*)?$/.exec(string);
          var value = {
            connType: matches[1],
          };
      ...
      removedList: {
        value: function (string, rm) {
          // wtf: into rm last member only received O_o
          if (!rm) return [];
        ...
        string: function (list) {
          ...
          var removing = _.pluck(
              _.where(figure.$primitives, { removed: true, type: "face" }),
              "removeIndex"
            ),
            groups = _.reduce(
              figure.$primitives,
              function (memo, item) {
                if (
                  item.removed &&
                  (item.type == "face" ||
                    _.all(item.$super.face, _.property("removed")))
                ) {
                  memo.push({
                    ...
                    faces:
                      item.type == "face"
                        ? [item.removeIndex]
                        : _.pluck(item.$super.face, "removeIndex"),
                  });
                }
                return memo;
              },
          ...
          while (removing.length > 0) {
            var group = _.reduce(
                groups,
                function (best, group) {
                  return best && best.faces.length >= group.faces.length
                    ? best
                    : group;
                },
                null
      ...
      tentNetBeginFace: {
        ...
        string: function (beginFaceIndex) {
          var face = _.isEmpty(beginFaceIndex) ? "" : "+" + beginFaceIndex,
            texture = _.isEmpty(this.tentNetTextureName)
              ? ""
              : "+" + this.tentNetTextureName;
  ...
  if (CALC_PATH == document.location.pathname) {
    ...
    if (!string && IS_OFFLINE) {
      document.location.pathname.replace(
        /^.*\$(\d+)_([^$/]+)(?:\$([^ %/]+))?(?:(?: |%20)\(\d+\))?\.html$/,
        function (pathname, numerator, more, removed) {
...
form.figureUrl = ko.computed(function () {
  return (
    "//acidome.com/lab/calc/" +
    (form.urlFragment() ? "#" + form.urlFragment() : "")
  );
});
...
function updateTitle(spec) {
  ...
  var hasRemoved = /~/.test(spec),
    title =
      spec.split("~")[0].replace(/_/g, " ") +
      (hasRemoved ? " (" + __("modified") + ")" : "");
...
function onFormChange() {
  ...
  form.state.fullerenType = _.contains(
    ["none", null, false, undefined],
    form.state.fullerenType
  )
    ? false
    : form.state.fullerenType;
  // fix detail type
  ...
  if (_.isEmpty(form._lastState)) {
    ...
    if (
      modeIndex < _.indexOf(MODES, "cover") &&
      !_.isEmpty(form.removedList())
    ) {
      viewer.mode("cover");
    }

    if (modeIndex < _.indexOf(MODES, "tent") && !!form.tentNetBeginFace()) {
      viewer.mode("tent");
    }
    ...
    _.compact([
      ...
      },
      {
        ...
        process: function () {
          if ("II" == state.subdivClass) {
            ...
            figure = new Figure[
              "Octohedron" == state.base ? "Octohedron" : "Icosahedron"
            ]({
              axis: AXIS,
              symmetry: state.symmetry,
            });
        ...
        process: function (figure) {
          ...
          switch (subdivClass) {
                ...
                case "Kruschke":
                  ...
                  */
                  ...
                  _.chain(figure.$primitives.get())
                    ...
                    .filter(
                      (l) => l.$points[0].pptPoint || l.$points[1].pptPoint
                    )
                    .each((line) => {
            ...
            case "II":
              ...
              if ("Octohedron" == state.base) {
                // splitFaces_updateToClassII() not for Octo
                return figure;
              }
            ...
            case "III":
              return figure.splitFaces_updateToClassIII(
                subdivAll[1],
                subdivAll[2]
              );
        ...
        process: function (figure) {
          ...
          _.each(figure.subs("face"), function (face) {
            ...
            if (!_.isEmpty(bad)) {
              ...
              badFactor = Math.max(
                badFactor,
                _.min(_.compact(_.pluck(bad, "badMax")))
              );
            }
          });
          if (badFactor) {
            console.error(
              "bad factor",
              badFactor * state.radius * (1000).toFixed(2),
              "mm"
            );
          }
      ...
      },
      {
        ...
        process: function (figure) {
          ...
          _.each(
            _.groupBy(figure.$primitives, "type"),
          ...
          if ("height" === state.partialMode) {
            ...
            const ZERO_DIR = 270,
              directionAngleFn = (deltaDegrees) => {
                return (
                  (Math.PI / 180) *
                  (ZERO_DIR - Number(state.doorGroup.direction) + deltaDegrees)
                );
              },
              ...
              topDirHorizComponent = Math.sqrt(
                topDirLength * topDirLength - topY * topY
              ),
              topPlane = new Plane(
                _.extend(horizVector(0).scale(-topY), {
            ...
            const baseDoorCL = figure
              .CuttingLine("base & door")
              .begin()
            ...
            baseDoorCL.whereMark({ base: -1 }, (face) => {
              //face.live = false;
              figure.safeRemoveMember(face);
            });

            0 !== doorWidth && baseDoorCL.stage("common").sewBack().relations();
            ...
            const PRECISION_THRESHOLD_REL = 2.5 / 1000 / form.state.radius,
              dirAngle =
                (Math.PI / 180) *
                (ZERO_DIR - Number(state.doorGroup.direction)),
              pointsByPlane = (plane, indoorFilter) => {
                return _.chain(figure.$points)
                  .filter(
                    (p) => Math.abs(plane.result(p)) < PRECISION_THRESHOLD_REL
                  )
                  .filter(
                    (p) =>
                      ((p._sides = []) && false === indoorFilter) ||
                  ...
                  .map((p) => {
                    return (
                      _.all(
                        ...
                        (plane, side) => {
                          if (
                            Math.abs(plane.result(p)) < PRECISION_THRESHOLD_REL
                          ) {
                            p._sides.push(side);
                          }
                          return (
              ...
              pointsBySide = {
                ...
                top: pointsByPlane(topPlane)
                  .each((p) => {
                    p.copy(
                      Vector.rotate(p, -Math.acos(Math.abs(topPlane.B)), "z")
                    ); // if(!) door's top > 0
                    p.copy(Vector.rotate(p, -Math.PI / 2, "x")); // (x,z) => (x,y)
                  })
                  .sortBy((p) => -p.y)
            ...
            form.doorGroup.sides(
              _.map(pointsBySide, (points, side) => {
                ...
                const range = ["x", "y", "z"].reduce((range, comp) => {
                  range[comp] = {
                ...
                const circleList = points
                  .slice(1)
                  .flatMap((B, index) => {
                    const A = points[index],
                      sideLength = A.distance(B),
                      holeCount = Math.max(
                        Math.ceil(sideLength / MAX_HOLES_INDENT),
                        1
                      );

                    return _.range(0, holeCount).map((i) => {
                      return A.clone()
                        .scale((holeCount - i) / holeCount)
            ...
            console.log(
              "pre-stat",
              _.countBy(figure.$primitives, "type"),
              figure.$points.length
            );
          } else if (
            state.subdivClass == "I" &&
            !state.fullerenType &&
            state.symmetry == "Pentad"
          ) {
      ...
      },
      {
        ...
        process: function (figure) {
          var Connector = Product.Connector[state.connType],
            ...
            Face =
              Product[state.fullerenType ? "Polygon" : "Triangle"]["Simple"];

          figure.$primitives = $(
            _.flatten(
              _.map(figure.$primitives, function (primitive) {
                switch (primitive.type) {
                  ...
                  case "face":
                    primitive.product = new (
                      primitive.$points.length === 3 && false ? Triangle : Face
                    )(primitive, {
                      R: state.radius,
                      bilateral: _.contains(
                        ["GoodKarma", "Semicone"],
                        state.connType
                      ),
                    });
      ...
      },
      {
        ...
        */
          ...
          if (state.connType == "GoodKarma" || state.connType == "Semicone") {
            ...
            figure.$primitives = figure.$primitives.filter(function () {
              return (
                0 || this.type != "line" || _.contains(faces, this.bindedFace)
              );
            });
          }
                ...
                [
                  {
                    ...
                    canvasAttr: {
                      ...
                      height: state.connType == "Semicone" ? 150 : 220,
                    },
                  },
                  {
                    type: "face",
                  ...
                  {
                    type: "vertex",
                    canvasAttr: { width: productAreaWidth / 3 },
                  },
                ],
                function (budget) {
                  budget.units = ko.computed(function () {
                    var dependsFrom = form.removedList();
                    return _.where(figure.$primitives, {
                      type: budget.type,
                      removed: false,
                    });
                  });
                  budget.sizeList = _.compact(
                    _.map(figure.stat[budget.type], function (size) {
                      ...
                      switch (budget.type) {
                        case "vertex":
                          var sample = _.sortBy(size.collect, function (one) {
                            return -(
                              // find with max count of live lines around
      ...
      },
      {
        ...
        process: function (figure) {
          ...
          const lineId = (a, b) => `${a}=>${b}`;

          // const ferrocementSupport = figure.$primitives
          //   .toArray()
          //   .filter(({ type, removed }) => type === "face" && !removed)
          //   .flatMap((face) => {
          //     const vertexIdsMap = face.$points
          //       .toArray()
          //       .reduce((all, point) => {
          //         const pointId = vectorId(coerce(point));
          ...
          //     const vertexIds = Object.keys(vertexIdsMap);

          //     return face.$sub.line.toArray().map(({ $points, ...rest }) => {
          //       const linePointIds = $points
          //         .toArray()
          //         .map((point) => vectorId(coerce(point)));

          //       const idOfLine = lineId(...linePointIds);

          //       const opposingVertexId = vertexIds.find(
          //         (faceVertex) => !linePointIds.includes(faceVertex)
          //       );

          //       const sum = $points.toArray().reduce(
          //         (all, current) => ({
          //           x: all.x + current.x,
          //           y: all.y + current.y,
          //           z: all.z + current.z,
          ...
          //       const lineMidPoint = coerce({
          //         x: sum.x / 2,
          //         y: sum.y / 2,
          //         z: sum.z / 2,
          //       });

          //       const lineMidPointId = vectorId(lineMidPoint);
          ...
          // console.log({ ferrocementSupport });

          const coercedPoints = Object.fromEntries([
            ...figure.origin.$points.toArray().map(({ x, y, z }) => {
              return [vectorId({ x, y, z }), coerce({ x, y, z })];
            }),
            // ...ferrocementSupport.map(({ lineMidPointId, lineMidPoint }) => {
            //   return [lineMidPointId, lineMidPoint];
            // }),
          ]);
          ...
          const rawElements = figure.$primitives
            .toArray()
          ...
          // const ferrocementSupportElements = ferrocementSupport.map(
          //   ({ lineMidPointId, lineMidPoint, opposingVertexId }) => {
          //     const aId = lineMidPointId;
          //     const bId = opposingVertexId;

          //     const opposingVertex = coercedPoints[opposingVertexId];

          //     const lowestY = Math.min(lineMidPoint.y, opposingVertex.id);
          //     const lowestPoint = lineMidPoint.y < opposingVertex.y ? aId : bId;
          ...
          const allElements = [
            ...rawElements,
            // ...ferrocementSupportElements
          ];

          const indexedPoints = Object.fromEntries(
            Object.entries(
              allElements.reduce((all, { aId, bId }) => {
                return {
                  ...all,
          ...
          const calculateElements = () => {
            ...
            const ferrocementSupportCrossSection = 0.000012566;

            const inputSteelCrossSection = parseFloat(
              window.prompt(
                "Type the steel cross section area",
                defaultCrossSection
              )
            );

            const steelCrossSection = isNaN(inputSteelCrossSection)
              ? defaultCrossSection
              : inputSteelCrossSection;

            return allElements
              .map(({ aId, bId }) => {
                return [indexedPoints[aId].index, indexedPoints[bId].index];
              })
              .map((arr, index) => {
                const isFerrocementSupport = index >= rawElements.length;

                const consideredCrossSection = isFerrocementSupport
                  ? ferrocementSupportCrossSection
                  : steelCrossSection;

                return [
                  index + 1,
                  ...arr,
                  ...getMemberParams(consideredCrossSection),
                ].join("\t");
              })
              .join("\n");
          };
          ...
          const classifiedElements = Object.entries(
            Object.values(allElements).reduce(
              (all, { a, b, lowestY, lowestPoint }) => ({
                ...all,
                [lowestPoint]: {
                  a,
                  b,
          ...
          const calculatePoints = () =>
            Object.values(indexedPoints)
              .map(({ index, ...point }) => {
                const { x, y, z } = coerce(point);

                return [index, x * 10, y * 10, z * 10].join("\t");
              })
              .join("\n");

          const calculateRestraints = () => {
            const count = parseInt(
              window.prompt("Type the amount of restraints", 10)
            );

            const validCount = isNaN(count) ? 10 : count;

            return Object.values(indexedPoints)
              .sort((a, b) => b.index - a.index)
              .slice(0, validCount)
              .map(({ index }) => [index, "TRUE", "TRUE", "TRUE"].join("\t"))
              .join("\n");
          };

          const calculateLoads = () => {
            const defaultImposedLoadT = 5;
            const userImposedLoad = parseFloat(
              window.prompt("Type the imposed load (Ton)", defaultImposedLoadT)
            );
            const imposedLoadT = isNaN(userImposedLoad)
              ? defaultImposedLoadT
              : userImposedLoad;
            const imposedLoadN = imposedLoadT * 10000;

            const defaultLoadPoints = 5;
            const userLoadPoints = parseInt(
              window.prompt(
                "Type the amount of points to distribute the load",
                defaultLoadPoints
              )
            );
            const loadPoints =
              isNaN(userLoadPoints) || userLoadPoints === 0
                ? defaultLoadPoints
                : userLoadPoints;

            const defaultOmitFirst = 0;
            const userOmitFirst = parseInt(
              window.prompt(
                "Type the amount of points to omit from the top",
                defaultOmitFirst
              )
            );
            const omitFirst = isNaN(userOmitFirst)
              ? defaultOmitFirst
              : userOmitFirst;

            const singleLoad = imposedLoadN / loadPoints;

            return new Array(loadPoints)
              .fill(null)
              .map((_, index) =>
                [index + 1 + omitFirst, 0, singleLoad * -1, 0].join("\t")
              )
              .join("\n");
          };

          const resultToContainer = (text) => {
            const el = document.getElementById("structural-result");
          ...
          setTimeout(() => {
            $("#copy-result").on("click", () => {
              const text = $("#structural-result").html();
            ...
            $("#copy-points").on("click", () => {
              ...
              resultToContainer(points);
            });
            $("#copy-members").on("click", () => {
              const elements = calculateElements();

              resultToContainer(elements);
            });
            $("#copy-restraints").on("click", () => {
              const restraints = calculateRestraints();

              resultToContainer(restraints);
            });
            $("#copy-loads").on("click", () => {
              const loads = calculateLoads();

              resultToContainer(loads);
            });
          }, 50);
      ...
      },
      {
        ...
        process: function (figure) {
          ...
          var diff = _.reduce(
            state,
            function (diff, value, key) {
              if (form._lastState[key] !== state[key]) diff[key] = value;
              return diff;
            },
            {}
          );
...
!(function () {
  var $progress = $(".geodesic .progress"),
    progressTotal;
